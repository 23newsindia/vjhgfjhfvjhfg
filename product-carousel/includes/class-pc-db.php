...
class PC_DB {
 ...
 public static function delete_carousel($id) {
    global $wpdb, $pc_cache;
    
    // Get carousel data before deletion
    $carousel = self::get_carousel_by_id($id);
    if (!$carousel) return false;

    // Clear all caches for this carousel
    $pc_cache->clear_carousel_cache($id);
    ...
    // Clear any cached shortcode output in posts
    $wpdb->query(
        $wpdb->prepare(
            "DELETE FROM {$wpdb->postmeta} 
             WHERE meta_key LIKE %s",
            '_pc_shortcode_cache_' . $carousel->slug . '%'
        )
    );

    // Trigger cleanup hooks
    do_action('pc_carousel_deleted', $id);
    return true;
}
