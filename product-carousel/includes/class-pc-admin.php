...
class PC_Admin {
    ...
    try {
        ...
        if (!empty($_POST['carousel_id'])) {
            ...
            if ($result !== false) {
                ...
                do_action('pc_carousel_settings_updated', $new_id);
            }
        }
   ...
   public function delete_carousel_ajax() {
    try {
        ...
        $carousel_id = absint($_POST['id'] ?? 0);
        if (!$carousel_id) {
            throw new Exception(__('Invalid carousel ID', 'product-carousel'));
        }

        // Delegate to PC_DB (handles cache+DB cleanup)
        if (!PC_DB::delete_carousel($carousel_id)) {
